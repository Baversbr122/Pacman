...
class NodeGroup(object):
        ...
        for row in list(range(data.shape[0])):
            for col in list(range(data.shape[1])):
                if data[row][col] in self.nodeSymbols:
                    x, y = constructKey(col + xoffset, row + yoffset)
    ...
    def connectHorizontally(self, data, xoffset=0, yoffset=0):
        for row in list(range(data.shape[0])):
            ...
            for col in list(range(data.shape[1])):
                if data[row][col] in self.nodeSymbols:
                    if key is None:
                        key = constructKey(col + xoffset, row + yoffset)
                    else:
                        otherkey = constructKey(col + xoffset, row + yoffset)
    ...
    def setPortalPair(self, pair1, pair2):
        key1 = constructKey(*pair1)
        key2 = constructKey(*pair2)
    ...
    def createHomeNodes(self, xoffset, yoffset):
        ...
        self.homekey = constructKey(xoffset + 2, yoffset)
        return self.homekey

    def connectHomeNodes(self, homekey, otherkey, direction):
        key = constructKey(*otherkey)
